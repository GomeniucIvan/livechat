...
namespace Smartstore.Core.Localization
{
    ...
    public partial class LanguageService : AsyncDbSaveHook<BaseEntity>, ILanguageService
    {
        ...
        public virtual List<Language> GetAllLanguages(bool includeHidden = false, bool tracked = false)
        {
        ...
        public virtual async Task<List<Language>> GetAllLanguagesAsync(bool includeHidden = false, bool tracked = false)
        {
        ...
        public virtual bool IsPublishedLanguage(int languageId)
        {
            if (languageId <= 0)
                return false;

            var map = GetStoreLanguageMap();
            if (map.ContainsKey(_storeContext.CurrentStore.Id))
            {
                return map[_storeContext.CurrentStore.Id].Any(x => x.Id == languageId);
            }

            return false;
        }

        public virtual async Task<bool> IsPublishedLanguageAsync(int languageId)
        {
            if (languageId <= 0)
                return false;
        ...
        public virtual bool IsPublishedLanguage(string seoCode)
        {
            if (seoCode.IsEmpty())
                return false;
            ...
            if (map.ContainsKey(_storeContext.CurrentStore.Id))
            {
                return map[_storeContext.CurrentStore.Id].Any(x => x.UniqueSeoCode == seoCode);
            }

            return false;
        }

        public virtual async Task<bool> IsPublishedLanguageAsync(string seoCode)
        {
            if (seoCode.IsEmpty())
                return false;
        ...
        public virtual string GetMasterLanguageSeoCode()
        {
            var map = GetStoreLanguageMap();
            if (map.ContainsKey(_storeContext.CurrentStore.Id))
            {
                return map[_storeContext.CurrentStore.Id].FirstOrDefault().UniqueSeoCode;
            }

            return null;
        }

        public virtual async Task<string> GetMasterLanguageSeoCodeAsync()
        {
            var map = await GetStoreLanguageMapAsync();
            if (map.ContainsKey(_storeContext.CurrentStore.Id))
            {
        ...
        public virtual int GetMasterLanguageId()
        {
            var map = GetStoreLanguageMap();
            if (map.ContainsKey(_storeContext.CurrentStore.Id))
            {
                return map[_storeContext.CurrentStore.Id].FirstOrDefault().Id;
            }

            return 0;
        }

        public virtual async Task<int> GetMasterLanguageIdAsync()
        {
            var map = await GetStoreLanguageMapAsync();
            if (map.ContainsKey(_storeContext.CurrentStore.Id))
            {
        ...
        protected virtual async Task<Multimap<int, LanguageStub>> GetStoreLanguageMapAsync()
        {
            var result = await _cache.GetAsync(STORE_LANGUAGE_MAP_KEY, async (o) =>
            {
                ...
                {
                    var languages = await GetAllLanguagesAsync(false);
                    if (!languages.Any())
                    {
                        // language-less stores aren't allowed but could exist accidentally. Correct this.
                        var firstStoreLang = (await GetAllLanguagesAsync(true)).FirstOrDefault();
