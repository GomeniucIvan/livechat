...
namespace Smartstore.Core.Localization
{
    public partial class LocalizationService : AsyncDbSaveHook<LocaleStringResource>, ILocalizationService
    {
        ...
        const string PUBLIC_CACHE_SEGMENT_KEY = "localization:{0}-1";
        ...
        protected virtual string BuildDtoCacheSegmentKey(int languageId)
        {
            return string.Format(PUBLIC_CACHE_SEGMENT_KEY, languageId);
        }
        ...
        public virtual async Task<IList<LocaleStringResourceDto>> GetPublicResourcesAsync(int languageId = 0)
        {
            languageId = languageId > 0 ? languageId : _workContext.Value.WorkingLanguage?.Id ?? 0;
            if (languageId == 0)
            {
                return new List<LocaleStringResourceDto>();
            }

            var cacheKey = BuildDtoCacheSegmentKey(languageId);
            ...
            return await _cache.GetAsync(cacheKey, async (o) =>
            {
                ...
                var publicResources = resources.Where(v => v.ResourceName.StartsWith("App."));

                var dict = new List<LocaleStringResourceDto>();

                foreach (var res in publicResources)
                {
                    dict.Add(new LocaleStringResourceDto()
                    {
