...
namespace Smartstore.Core.Localization
{
    public class LanguageResolver : ILanguageResolver
    {
        ...
        protected virtual async Task<Language> ResolveLanguageCore(Customer currentCustomer, HttpContext httpContext, bool async)
        {
            ...
            if (httpContext == null)
            {
                return await GetDefaultLanguage(customerLangId, async);
            }

            return
                // 1: Try resolve from route values or from request path
                await ResolveFromRoute(httpContext, async) ??
                // 2: Try resolve from determined customer lang id
                await ResolveFromCustomer(customerLangId, async) ??
                // 3: Try resolve from accept header
                await ResolveFromAcceptHeader(httpContext, customerLangId, currentCustomer, async) ??
                // 3: Get default fallback language
                await GetDefaultLanguage(customerLangId, async) ??
                // Should never happen
                throw new SmartException("At least one language must be active!");
        }

        protected virtual async Task<Language> ResolveFromRoute(HttpContext httpContext, bool async)
        {
            ...
            if (cultureCode.IsEmpty() || !_languageService.IsPublishedLanguage(cultureCode))
            {
                return null;
            }
        ...
        protected virtual async Task<Language> ResolveFromCustomer(int customerLangId, bool async)
        {
            if (customerLangId > 0 && _languageService.IsPublishedLanguage(customerLangId))
            {
                return async
        ...
        protected virtual async Task<Language> ResolveFromAcceptHeader(HttpContext httpContext, int customerLangId, Customer customer, bool async)
        {
            ...
            if (providerResult != null)
            {
                foreach (var culture in providerResult.Cultures)
                {
                    ...
                    if (language != null && _languageService.IsPublishedLanguage(language.Id))
                    {
                        return language;
                    }
                }
        ...
        protected virtual async Task<Language> GetDefaultLanguage(int customerLangId, bool async)
        {
            if (customerLangId == 0 || !_languageService.IsPublishedLanguage(customerLangId))
            {
                customerLangId = _languageService.GetMasterLanguageId();
            }
