...
namespace Smartstore.Core.Seo
{
    public partial class XmlSitemapGenerator : AsyncDbSaveHook<BaseEntity>, IXmlSitemapGenerator
    {
        ...
        private const string LockFileNamePattern = "sitemap-{0}.lock";
        ...
        private async Task<XmlSitemapPartition> GetSitemapPartAsync(int index, bool isRetry)
        {
            ...
            var exists = TryGetSitemapFile(language.Id, index, out var file);
            ...
            if (index > 0)
            {
                ...
                if (TryGetSitemapFile(language.Id, 0, out file))
                {
            ...
            var lockFilePath = GetLockFilePath(language.Id);
            ...
            {
                // No lock. Rebuild now.
                var buildContext = new XmlSitemapBuildContext(store, new[] { language }, _services.SettingFactory)
                {
        ...
        private bool TryGetSitemapFile(int languageId, int index, out IFile file)
        {
            var path = BuildSitemapFilePath(languageId, index);
        ...
        private string BuildSitemapFilePath(int languageId, int index)
        {
            var fileName = SitemapFileNamePattern.FormatInvariant(index);
            return PathUtility.Join(BuildSitemapDirPath(languageId), fileName);
        }

        private string BuildSitemapDirPath(int? languageId)
        {
            return _baseDir;
        }

        private string GetLockFilePath(int languageId)
        {
            var fileName = LockFileNamePattern.FormatInvariant(languageId);
        ...
        public virtual async Task RebuildAsync(XmlSitemapBuildContext ctx)
        {
            ...
            foreach (var language in ctx.Languages)
            {
                ...
                if (_lockFileManager.TryAcquireLock(lockFilePath, out var lockFile))
                {
                    ...
                    try
                    {
                        var sitemapDir = BuildSitemapDirPath(language.Id);
                        var data = new LanguageData
                        {
                            ...
                            TempDir = sitemapDir + "~",
                            FinalDir = sitemapDir,
                            BaseUrl = await BuildBaseUrlAsync(language)
                        };
        ...
        private async Task<string> BuildBaseUrlAsync(Language language)
        {
            ...
            {
                var defaultLangId = await _languageService.GetMasterLanguageIdAsync();
        ...
        private async Task SaveFinalAsync(LanguageData data)
        {
            ...
            int retries = 0;
            while (!TryGetSitemapFile(data.Language.Id, 0, out _))
            {
                if (retries > 20)
                {
                    break;
        ...
        public Task<bool> IsRebuildingAsync(int languageId)
        {
            return IsRebuildingAsync(GetLockFilePath(languageId));
        }
        ...
        public virtual bool IsGenerated(int languageId)
        {
            return TryGetSitemapFile(languageId, 0, out _);
        }

        public virtual void Invalidate(int? languageId)
        {
            var dir = BuildSitemapDirPath(languageId);
            _tenantRoot.TryDeleteDirectory(dir);
        }

        public virtual void InvalidateAll()
        {
            var dir = BuildSitemapDirPath(null);
