...
namespace Smartstore.Core.Configuration
{
    public class SettingFactory : ISettingFactory
    {
        ...
        /// <inheritdoc/>
        public ISettings LoadSettings(Type settingsType)
        {
            ...
            var cacheKey = SettingService.BuildCacheKeyForClassAccess(settingsType);
            ...
            return _cache.Get(cacheKey, o =>
            {
                ...
                {
                    var rawSettings = GetRawSettings(db, settingsType, false);
        ...
        /// <inheritdoc/>
        public Task<ISettings> LoadSettingsAsync(Type settingsType)
        {
            Guard.NotNull(settingsType, nameof(settingsType));
            Guard.HasDefaultConstructor(settingsType);
            ...
            return _cache.GetAsync(cacheKey, async (o) =>
            {
                ...
                using (GetOrCreateDbContext(out var db))
                {
                    var rawSettings = await GetRawSettingsAsync(db, settingsType, false);
                    return MaterializeSettings(settingsType, rawSettings);
                }
            }, independent: true, allowRecursion: true);
        }
        ...
        /// <inheritdoc/>
        public async Task<int> SaveSettingsAsync<T>(T settings) where T : ISettings, new()
        {
            // INFO: Let SettingService's hook handler handle cache invalidation
            using (GetOrCreateDbContext(out var db))
            {
                var numSaved = await SaveSettingsAsync(db, settings, true);
                if (numSaved > 0)
                {
                    // Prevent reloading from DB on next hit
                    await _cache.PutAsync(
                        SettingService.BuildCacheKeyForClassAccess(typeof(T)),
                        settings,
        ...
        /// Internal API.
        /// </summary>
        public static async Task<int> SaveSettingsAsync(SmartDbContext db, ISettings settings, bool overwriteExisting = true)
        {
            ...
            var rawSettings = await GetRawSettingsAsync(db, settingsType, tracked: true);
        ...
        private static IDictionary<string, Setting> GetRawSettings(SmartDbContext db, Type settingsType, bool tracked = false)
        {
            var list = db.Settings
                .ApplyTracking(tracked)
                .ApplyClassFilter(settingsType)
                .ApplySorting()
                .ToList();

            // Because the list is sorted, store-specific entries overwrite neutral ones.
            return list.ToDictionarySafe(k => k.Name, e => e, StringComparer.OrdinalIgnoreCase);
        }

        private static async Task<IDictionary<string, Setting>> GetRawSettingsAsync(SmartDbContext db, Type settingsType, bool tracked = false)
        {
